...
class AddRemindUseCase @Inject constructor(
    private val repository: Repository,
    @IOPool
    configuration: Configuration,
) :
    UseCase<AddRemindUseCase.Request, AddRemindUseCase.Response>(configuration) {

    class Response : UseCase.Response
    data class Request(val remindModel: RemindModel) : UseCase.Request

    override fun process(request: Request): Flow<Response> = flow{
        emit(repository.addRemind(request.remindModel))
    }.map { Response() }
